...
function App() {
  ...
  const moisturePercent = Math.min(
    100,
    Math.max(0, 100 - (sensorData.soil_moisture / 4095) * 100)
  );
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ marginBottom: 30, textAlign: 'center' }}>
        <input
          type="text"
          placeholder="Enter ESP32 IP address (e.g. 192.168.137.222)"
          value={ipAddress}
      ...
      {fetching ? (
        <div style={styles.cardContainer}>
          ...
          {/* Soil Moisture */}
          <div style={styles.card}>
            <FaSeedling size={36} color="#52c41a" />
            <h2>Soil Moisture</h2>
            <div style={{ width: 140, margin: '0 auto' }}>
              <CircularProgressbar
                value={moisturePercent}
                maxValue={100}
                text={`${moisturePercent.toFixed(0)}%`}
                styles={buildStyles({
                  pathColor: "#52c41a",
                  textColor: "#52c41a",
                  trailColor: "#e6ffe6",
        ...
        <p style={{ textAlign: 'center', color: '#999' }}>
          Enter the ESP32 IP address to begin fetching data.
        </p>
      )}
    </div>
  );
